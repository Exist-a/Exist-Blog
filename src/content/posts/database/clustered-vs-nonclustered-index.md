...
还有一个常被忽略的区别：在 B 树里，叶子节点并不会重复出现上层节点已经出现过的 key，每个 key 在整棵树里只出现一次；而 B+ 树的叶子节点则会包含所有索引键，上层出现过的 key 在叶子层会再出现一遍，仅起索引作用。这个差别带来的一个实际收益是，B+ 树的非叶子节点因为不存数据，同样的页大小可以装下更多的索引键，从而让整棵树的高度更低、磁盘 I/O 更少。

聚簇索引和非聚簇索引都是基于 B+ 树实现的。MySQL 选择 B+ 树而不是 B 树，其中一个重要原因正是 B+ 树的叶子节点通过双向链表相连，这让范围查询和顺序扫描都可以沿着链表高效完成。
...
- B 树每个节点都存数据，且 key 在整棵树里只出现一次；B+ 树只有叶子节点存数据，叶子节点用双向链表相连，且上层出现过的 key 在叶子层会再出现，这让范围查询更高效，也让非叶子节点能装下更多 key、降低树的高度。
